...
class AntColony(object):
	...
	def tour_const(self):
		...
		self.create_node()
		self.reset_color()
		pygame.draw.rect(self.screen, self.CUSTOMER , [self.size[0] - 200, 50, 30,30])
		cus = self.myFont.render("Customer Node",1, self.CUSTOMER)
		self.screen.blit(cus,(self.size[0] - 160,50))
		pygame.draw.rect(self.screen, self.DISPOSAL , [self.size[0] - 200, 90, 30,30])
		dis = self.myFont.render("Disposal Node",1, self.DISPOSAL)
		self.screen.blit(dis,(self.size[0] - 160,90))
		pygame.draw.rect(self.screen, self.DEPOT , [self.size[0] - 200, 130, 30,30])
		depo = self.myFont.render("Depot Node",1, self.DEPOT)
		self.screen.blit(depo,(self.size[0] - 160,130))
		pygame.display.flip()
